<?php
/**
 * MIGRAÇÃO INDEPENDENTE - SEM DEPENDÊNCIAS EXTERNAS
 * Versão que cria sua própria conexão com banco
 */
...
echo "<h2>🔄 MIGRAÇÃO INDEPENDENTE DO BANCO</h2>";

// Configurações do banco (ajuste conforme sua hospedagem)
$config_banco = [
    'host' => 'localhost',
    'dbname' => 'u894209272_planos_aula', // Ajuste conforme seu banco
...
echo "<p><strong>Banco:</strong> {$config_banco['dbname']}</p>";
echo "<p><strong>Usuário:</strong> {$config_banco['user']}</p>";
echo "<p><strong>Charset:</strong> {$config_banco['charset']}</p>";
echo "</div>";

// Tentar conectar ao banco
try {
    $dsn = "mysql:host={$config_banco['host']};dbname={$config_banco['dbname']};charset={$config_banco['charset']}";
    $pdo = new PDO($dsn, $config_banco['user'], $config_banco['pass']);
    ...
    echo "<li>Confirme o nome do banco: <code>{$config_banco['dbname']}</code></li>";
    echo "<li>Confirme o usuário: <code>{$config_banco['user']}</code></li>";
    echo "<li>Confirme a senha (atualmente vazia)</li>";
    echo "<li>Verifique se o host está correto: <code>{$config_banco['host']}</code></li>";
    echo "</ol>";
...
try {
    ...
    echo "<p>✅ Encontradas " . count($tabelas_existentes) . " tabelas no banco</p>";
...
if (!in_array('pre_cadastros_controle', $tabelas_existentes)) {
    ...
    try {
        $pdo->exec($sql);
        echo "<p style='color: green;'>✅ Tabela <strong>pre_cadastros_controle</strong> criada com sucesso</p>";
    } catch (Exception $e) {
...
if (!in_array('configuracoes_sistema', $tabelas_existentes)) {
    ...
    try {
        $pdo->exec($sql);
        echo "<p style='color: green;'>✅ Tabela <strong>configuracoes_sistema</strong> criada com sucesso</p>";
    } catch (Exception $e) {
...
if (!in_array('mensalidades', $tabelas_existentes)) {
    ...
    try {
        $pdo->exec($sql);
        echo "<p style='color: green;'>✅ Tabela <strong>mensalidades</strong> criada com sucesso</p>";
    } catch (Exception $e) {
...
if (in_array('alunos', $tabelas_existentes)) {
    ...
    foreach ($colunas_alunos as $coluna => $definicao) {
        try {
            ...
            if (!$existe) {
                ...
                $pdo->exec($sql);
                echo "<p style='color: green;'>✅ Coluna <strong>$coluna</strong> adicionada à tabela alunos</p>";
            } else {
                echo "<p>ℹ️ Coluna <strong>$coluna</strong> já existe na tabela alunos</p>";
            }
        } catch (Exception $e) {
    ...
    echo "<p style='color: orange;'>⚠️ Tabela <strong>alunos</strong> não encontrada - pulando adição de colunas</p>";
}
...
if (in_array('usuarios', $tabelas_existentes)) {
    try {
        ...
        if (!$existe) {
            ...
            $pdo->exec($sql);
            echo "<p style='color: green;'>✅ Coluna <strong>tipo</strong> adicionada à tabela usuarios</p>";
        } else {
            echo "<p>ℹ️ Coluna <strong>tipo</strong> já existe na tabela usuarios</p>";
        }
    } catch (Exception $e) {
        echo "<p style='color: orange;'>⚠️ Erro ao adicionar coluna tipo: " . $e->getMessage() . "</p>";
    }
} else {
    echo "<p style='color: orange;'>⚠️ Tabela <strong>usuarios</strong> não encontrada - pulando adição de colunas</p>";
}
...
if (in_array('configuracoes_sistema', $tabelas_existentes)) {
    ...
    foreach ($configuracoes as $config) {
        try {
            ...
            ");
            $stmt->execute($config);
            echo "<p style='color: green;'>✅ Configuração <strong>{$config['chave']}</strong> inserida/atualizada</p>";
        } catch (Exception $e) {
            echo "<p style='color: orange;'>⚠️ Erro ao inserir configuração {$config['chave']}: " . $e->getMessage() . "</p>";
        }
    }
} else {
    echo "<p style='color: orange;'>⚠️ Tabela <strong>configuracoes_sistema</strong> não encontrada - pulando configurações</p>";
}
