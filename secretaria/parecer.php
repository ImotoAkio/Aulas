...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Buscar o ano letivo da turma para usar como período
  $periodo = '';
  if ($id_turma) {
    $stmt_ano = $pdo->prepare("SELECT ano_letivo FROM turmas WHERE id = ?");
    $stmt_ano->execute([$id_turma]);
    $periodo = $stmt_ano->fetchColumn();
  }
      ...
      if (empty($alunos_na_turma)) {
        ...
        if ($created_count > 0 || $skipped_count > 0) {
          ...
          if ($skipped_count > 0) {
            ...
            if ($feedback_type == '')
              $feedback_type = 'info'; // Se nada foi criado, mas algo ignorado
          }
          if ($error_count > 0) {
...
try {
  $turmas = $pdo->query("SELECT id, nome, ano_letivo FROM turmas ORDER BY nome")->fetchAll(PDO::FETCH_ASSOC);
...
try {
  // Filtros
  $filtro_turma = filter_input(INPUT_GET, 'filtro_turma', FILTER_VALIDATE_INT);
  $filtro_unidade = filter_input(INPUT_GET, 'filtro_unidade', FILTER_SANITIZE_STRING);
  $busca_aluno = filter_input(INPUT_GET, 'busca_aluno', FILTER_SANITIZE_STRING);
  ...
  if ($filtro_turma) {
    $sql_finalizados .= " AND p.id_turma = :filtro_turma";
    $params_finalizados[':filtro_turma'] = $filtro_turma;
  }

  if ($filtro_unidade) {
    $sql_finalizados .= " AND p.unidade = :filtro_unidade";
    $params_finalizados[':filtro_unidade'] = $filtro_unidade;
  }

  if ($busca_aluno) {
    $sql_finalizados .= " AND a.nome LIKE :busca_aluno";
    $params_finalizados[':busca_aluno'] = "%$busca_aluno%";
  }
  ...
  $stmt_finalizados->execute($params_finalizados);
...
<head>
  ...
  <!-- plugins:css -->
  <link rel="stylesheet" href="<?php echo getAssetUrl("assets/vendors/mdi/css/materialdesignicons.min.css"); ?>" <link
    rel="stylesheet" href="<?php echo getAssetUrl("assets/vendors/ti-icons/css/themify-icons.css"); ?>" <link
    rel="stylesheet" href="<?php echo getAssetUrl("assets/vendors/css/vendor.bundle.base.css"); ?>" <link
    rel="stylesheet" href="<?php echo getAssetUrl("assets/vendors/font-awesome/css/font-awesome.min.css"); ?>" <!--
    endinject -->
  ...
  <style>
    ...
    .feedback-message {
      ...
      border-radius: 8px;
      margin-bottom: 10px;
      /* Espaço entre múltiplas mensagens */
      font-weight: bold;
      opacity: 0;
      /* Começa invisível */
              ...
              <div class="card">
                <div class="card-body">
                  ...
                  <form action="parecer.php" method="POST" class="forms-sample">
                      ...
                      <select class="form-control" id="id_turma" name="id_turma" required>
                        ...
                        <?php foreach ($turmas as $turma): ?>
                          <option value="<?php echo htmlspecialchars($turma['id']); ?>">
                            <?php echo htmlspecialchars($turma['nome']) . ' - ' . htmlspecialchars($turma['ano_letivo']); ?>
                          </option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                    ...
                    <button type="submit" class="btn btn-gradient-primary me-2">Criar e Designar Parecer(es) por
                      Turma</button>
        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            ...
            <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title"><i class="mdi mdi-cloud-download"></i> Baixar Pareceres em Lote</h4>
                  <p class="card-description">Gere um único arquivo PDF contendo todos os pareceres de uma turma e unidade.</p>
                  
                  <form action="boletim/gerar_pdf_parecer_lote.php" method="GET" target="_blank" class="forms-sample">
                    <div class="row">
                        <div class="col-md-5">
                            <div class="form-group">
                              <label for="lote_turma">Turma</label>
                              <select class="form-control" id="lote_turma" name="id_turma" required>
                                <option value="">Selecione uma turma</option>
                                <?php foreach ($turmas as $turma): ?>
                                      <option value="<?php echo htmlspecialchars($turma['id']); ?>">
                                          <?php echo htmlspecialchars($turma['nome']) . ' - ' . htmlspecialchars($turma['ano_letivo']); ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                              <label for="lote_unidade">Unidade</label>
                              <select class="form-control" id="lote_unidade" name="unidade" required>
                                <option value="">Selecione a unidade</option>
                                <?php foreach ($unidades_enum as $unidade_val): ?>
                                      <option value="<?php echo htmlspecialchars($unidade_val); ?>">
                                          <?php echo htmlspecialchars($unidade_val) . "° Unidade"; ?>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <button type="submit" class="btn btn-gradient-info btn-block">
                                    <i class="mdi mdi-file-pdf"></i> Baixar Todos
                                </button>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  ...
                  <p class="card-description">Lista de pareceres que já foram concluídos por professores ou
                    coordenadores.</p>
                  
                  <div class="mb-4">
                      <form method="GET" class="row align-items-end">
                          <div class="col-md-4">
                              <label for="busca_aluno">Buscar Aluno</label>
                              <input type="text" class="form-control" id="busca_aluno" name="busca_aluno" value="<?php echo htmlspecialchars($busca_aluno ?? ''); ?>" placeholder="Nome do aluno">
                          </div>
                          <div class="col-md-3">
                              <label for="filtro_turma">Filtrar por Turma</label>
                              <select class="form-control" id="filtro_turma" name="filtro_turma">
                          ...
                          <div class="col-md-3">
                              ...
                              <select class="form-control" id="filtro_unidade" name="filtro_unidade">
                                  <option value="">Todas as Unidades</option>
                                  <?php foreach ($unidades_enum as $unidade_val): ?>
                                          <option value="<?php echo $unidade_val; ?>" <?php echo (isset($filtro_unidade) && $filtro_unidade == $unidade_val) ? 'selected' : ''; ?>>
                                              <?php echo $unidade_val . "° Unidade"; ?>
                                          </option>
                                  <?php endforeach; ?>
                              </select>
                          </div>
                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        <?php if (empty($pareceres_finalizados)): ?>
                              <tr>
                                <td colspan="6" class="text-center">Nenhum parecer finalizado encontrado.</td>
                              </tr>
                        <?php else: ?>
                              <?php foreach ($pareceres_finalizados as $parecer): ?>
                                    <tr>
                                      <td><?php echo htmlspecialchars($parecer['nome_aluno']); ?></td>
                                      <td><?php echo htmlspecialchars($parecer['nome_turma'] ?? 'N/A'); ?>
                                        (<?php echo htmlspecialchars($parecer['ano_letivo'] ?? 'N/A'); ?>)</td>
                                      ...
                                      <td>
                                        <a href="ver_parecer.php?id_aluno=<?php echo htmlspecialchars($parecer['id_aluno']); ?>&periodo=<?php echo urlencode($parecer['periodo']); ?>"
                                          class="btn btn-info btn-sm" target="_blank" title="Ver Detalhes">
                                          <i class="mdi mdi-eye"></i>
                                        </a>
                                        <a href="boletim/gerar_pdf_parecer.php?id_aluno=<?php echo htmlspecialchars($parecer['id_aluno']); ?>&periodo=<?php echo urlencode($parecer['periodo']); ?>" 
                                           class="btn btn-danger btn-sm" target="_blank" title="Baixar PDF">
