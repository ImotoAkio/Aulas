...
try {
    $stmt = $pdo->prepare("
        SELECT a.id, a.nome, a.codigo_pre_cadastro, a.status_cadastro,
               t.nome as turma_nome
        FROM alunos a
        LEFT JOIN turmas t ON a.turma_id = t.id
        WHERE a.codigo_pre_cadastro IS NOT NULL
        ORDER BY a.id DESC
        LIMIT 5
    ");
    $stmt->execute();
    $alunos_teste = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    $alunos_teste = [];
    ...
    <div class="test-container">
        ...
        <div class="test-card">
            <div class="test-body">
                ...
                <form method="POST" action="teste_webhook.php">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="d-flex gap-2">
                                    <button type="submit" class="btn btn-test flex-fill">
                                        <i class="mdi mdi-send"></i> Testar Webhook
                                    </button>
                                    <button type="button" class="btn btn-secondary flex-fill" onclick="limparFormulario()">
        ...
        <div class="test-card">
            <div class="test-body">
                <h3><i class="mdi mdi-account-group"></i> Alunos Disponíveis para Teste</h3>
                <p class="text-muted">Alunos com código de pré-cadastro que podem ser usados para teste</p>
                
                <?php if (empty($alunos_teste)): ?>
                <div class="alert alert-info">
                    <i class="mdi mdi-information"></i> Nenhum aluno com código de pré-cadastro encontrado.
                </div>
                <?php else: ?>
                <div class="table-responsive">
                    <table class="table table-custom">
                        ...
                        <tbody>
                            <?php foreach ($alunos_teste as $aluno): ?>
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-primary" onclick="usarAlunoParaTeste('<?php echo htmlspecialchars($aluno['nome']); ?>', '<?php echo htmlspecialchars($aluno['codigo_pre_cadastro']); ?>')">
                                        <i class="mdi mdi-check"></i> Usar
                                    </button>
                                </td>
                            </tr>
                            <?php endforeach; ?>
  ...
  "timestamp": "2025-01-27 14:30:00"
    ...
    <script>
        function limparFormulario() {
            ...
            document.getElementById('telefone_teste').value = '';
            document.getElementById('aluno_teste').value = '';
        }
        
        function usarAlunoParaTeste(nome, codigo) {
            document.getElementById('aluno_teste').value = nome;
            // Opcional: atualizar o link de teste também
            alert('Aluno "' + nome + '" selecionado para teste!\nCódigo: ' + codigo);
        }
